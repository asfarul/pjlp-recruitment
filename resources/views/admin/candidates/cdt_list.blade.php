                                ...
                                {{-- <select name="period_id" id="period_id" class="form-control">
                                    ...
                                    {{-- @foreach ($periods as $period)
                                        <option value="{{ $period->id }}">
                                            ...
                                            {{ date('d/m/Y', strtotime($period->end_date)) }} : {{ $period->description }}
                                        </option>
                                    @endforeach
                                </select> --}}
                            </div>
                        ...
                        <tbody>
                            @foreach ($candidates as $candidate)
                                <tr>
                                    ...
                                    <td class="text-center"><img src="{{ url('/uploads') . $candidate->foto }}"
                                            height="50">
                                    </td>
                                    <td>{{ $candidate->nama }}</td>
                                    <td>{{ $candidate->nik }}</td>
                                    <td>{{ $candidate->vacancy && $candidate->vacancy->periode ? date('d/m/Y', strtotime($candidate->vacancy->periode->start_date)) . ' s/d ' . date('d/m/Y', strtotime($candidate->vacancy->periode->end_date)) : '-' }}
                                    </td>
                                    <td>
                                        {{ $candidate->vacancy ? $candidate->vacancy->title : '' }}
                                    </td>
                                    <td>{{ $candidate->vacancy && $candidate->vacancy->dinas ? $candidate->vacancy->dinas->opd : '' }}
                                    </td>
                                    <td class="text-center"><span
                                            class="label label-info">{{ $candidate->candidate_status ? $candidate->candidate_status->candidate_status : '' }}</span>
                                    </td>
                                    <td class="text-center">
                                        <div class="btn-group">
                                            ...
                                            <ul class="dropdown-menu text-right">
                                                ...
                                                <li><a href="{{ url('/sysadmin/pelamar/download/' . $candidate->id) }}">Download
                                                        Semua</a></li>
                                                <li><a href="{{ url('/uploads') . $candidate->ktp }}"
                                                        target="_blank">KTP</a></li>
                                                <li><a href="{{ url('/uploads') . $candidate->ijazah }}"
                                                        target="_blank">Ijazah</a>
                                                </li>
                                                <li><a href="{{ url('/uploads') . $candidate->transkrip }}"
                                                        target="_blank">Transkrip
                                                        Nilai</a></li>
                                                ...
                                                @if (!empty($candidate->sertifikat))
                                                    <li><a href="{{ url('/uploads') . $candidate->sertifikat }}"
                                                            target="_blank">Sertifikat</a></li>
                                                @endif
                                            </ul>
                                        </div>
                                    </td>
                                        ...
                                        <td class="text-center">
                                            <div class="btn-group">
                                                @permission('show-candidates')
                                                    <a href="javascript:;" class="btn btn-xs btn-warning" data-toggle="modal"
                                                        data-target="#detail"
                                                    ...
                                                    @permission('edit-candidates')
                                                        <a href="{{ route('pelamar.edit', $candidate->id) }}"
